...
def timber_piles_terrain(terrain_ground, terrain_ceiling, timber_spacing=1.0, timber_size=0.3, pile_height=1.2, hanging_obstacles=False, position_noise=0.2, height_noise=0.1, spawn_platform_height=None):
    ...
    Parameters:
        ...
        height_noise: random height perturbation [meters]
        spawn_platform_height: height of spawn platform [meters], None = use pile_height
    """
    # Convert to discrete units
    ...
    # Create flat platform for spawn area (use specified height or fall back to pile height)
    platform_height = pile_height_units if spawn_platform_height is None else int(spawn_platform_height / terrain_ground.vertical_scale)
...
def complex_maze_terrain(terrain_ground, terrain_ceiling, complexity=0.5):
    ...
    # 3. Add "Confined Roof" patches - Randomly lower ceiling in some areas
    num_low_ceilings = 10
...
class TerrainConfined:
    ...
    def __init__(self, cfg: LeggedRobotCfg.terrain, num_robots) -> None:
        ...
        # Confined terrain proportions: [corridor, timber, column, maze, tunnel, gap, corridor_fallback]
    ...
    def make_confined_terrain(self, choice, difficulty):
        ...
        if self.corridor_only:
            ...
            if corridor_uniform_width:
                ...
                corridor_width = corridor_width_base - 0.7 * difficulty
            corridor_obstacle_density = 0.06 + 0.18 * difficulty
            corridor_obstacle_size = 0.18 + 0.16 * difficulty
            corridor_obstacle_height = 0.18 + 0.16 * difficulty
            corridor_num_turns = 0 if corridor_uniform_width else int(difficulty * 3)
        ...
        if choice < self.proportions[0]:
            ...
            # Wall with gap terrain (fallback)
