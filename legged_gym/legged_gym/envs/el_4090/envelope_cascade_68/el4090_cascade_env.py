"""EL_4090_CASCADE_68: 68-dim SE2 locomotion driven by EA2 point-cloud perception.
...
1. ``super().post_physics_step()`` — SE2 refreshes buffers, resamples
   ...
   finished envs and builds the 68-dim SE2 observation.
...
4. The bridge maps raw actions → ``params5`` → derived ``condition8``;
   ...
   envelope is visible to the locomotion policy in the same step (the obs
   centres ``dof_pos`` on the morphology preset, so it depends on the
   condition even though the envelope itself is not observed).

``ea2.enable=False`` restores pure SE2 behaviour (randomly sampled
envelopes) — used as an A/B baseline.

Structural twin of ``envelope_cascade_83`` (which freezes the legacy
83-dim SE2 contract); this package freezes the current 68-dim contract.
"""
...
from legged_gym.envs.el_4090.envelope_cascade_68.ea2_perception import Ea2Perception
...
class EL_4090_CASCADE_68(EL_4090_SE2_68):
    ...
    def __init__(
        ...
        sim_device,
        headless,
        task_name: str = "el4090_cascade_68",
    ):
    ...
    def _init_cascade(self) -> None:
        ...
        if abs(self._cascade_bridge.fold_scale - float(ea2.fold_scale)) > 1e-9:
            raise ValueError(
                ...
                "envelope_cascade_68/checkpoints/README.md)"
            )
    ...
    def _resample_commands(self, env_ids: torch.Tensor) -> None:
        ...
        # sampled fallback path (mirrors EL_4090_SE2_68._resample_commands).
