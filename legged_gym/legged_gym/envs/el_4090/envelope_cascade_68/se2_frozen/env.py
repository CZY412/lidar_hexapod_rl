"""Frozen 68-dim SE2 (spider_envelop_2) environment, vendored for cascade_68.
...
cascade_68 checkpoints (``policy_1.pt``: 68-dim TorchScript input; HAA range
network input order) are pinned against, isolating the cascade from future
upstream evolution.  Do not modify except by an explicit, reviewed
re-vendor.  Only deviations from the verbatim source: this provenance header,
the class rename ``EL_4090_ENVELOP_2`` -> ``EL_4090_SE2_68``, and the config /
...
class EL_4090_SE2_68(EL_4090_ENVELOP):
