"""Frozen 83-dim SE2 (spider_envelop_2) environment, vendored for cascade_83.

Source: ``legged_gym/legged_gym/envs/el_4090/spider_envelop_2/el_4090.py``
at commit ``fd527a8`` (merge base with ``feat/el_4090_2``).  Upstream SE2 has
since removed the range priors from the policy observation (68-dim); this
frozen copy preserves the 83-dim contract that the cascade_83 checkpoints
(``policy_1.pt``: 83-dim TorchScript input; HAA range network input order)
were trained against.  Do not modify except by an explicit, reviewed
re-vendor.  Only deviations from the verbatim source: this provenance header,
the class rename ``EL_4090_ENVELOP_2`` -> ``EL_4090_SE2_83``, and the config /
envelope-condition imports re-pointed at this package's frozen copies.
"""
...
class EL_4090_SE2_83(EL_4090_ENVELOP):
