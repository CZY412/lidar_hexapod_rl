...
- 地图四周边界视为 occupied（或显式加边界墙），防止路径出界；边界是否建实体墙只影响可视化，不影响 occupancy 语义；
...
| 基元 | 说明 | 默认参数 |
|---|---|---|
| 墙体 | 长条矩形障碍 | 长 2~5m，厚 0.2~0.5m |
| 立柱/柱体 | 方形或圆形截面 | 边长/直径 0.4~0.8m |
| 窄通道 | 两段平行墙，中间留缝 | 缝宽 1.0~2.0m |
| 单侧墙群 | 只在路径一侧布置多段墙 | 数量 2~5 段 |
| 死角/U 形 | 三面墙围出凹槽 | 开口 1.0~1.5m |

- 障碍物高度统一设为 **1.5~2.0m**，保证 LiDAR 能稳定打到；
- 碰撞判定只看 2D 投影，不看高度；
- **障碍物与路径的距离不能过远**，否则没有碰撞训练信号；建议障碍物距路径中心约 `0.7~1.5m`，窄通道宽度 `1.0~2.0m`；
- 地图生成验收（验收时用**预采样验证路径**，不要依赖训练期每 env 路径；不满足则重生成，设最大尝试次数）：
  1. 预采样 8~16 对起点/终点跑 A*，要求全部（或 ≥80%）有解；
  2. 在这些验证路径上，至少 30% 的路径点到最近障碍距离 ∈ `[0.7, 1.5]m`；
  3. 本张地图的基元列表至少包含 1 处窄通道或单侧墙约束段。
...
- 生成后处理顺序：A* → line-of-sight 简化 → 按固定间距重采样（`0.2m`）→ 施加路径噪声（§2.2.6）→ 检查最小转弯半径 → 由切线给出每个路径点的 yaw；
- 最小转弯半径 `R_min ≥ 1.0m`（可配置）：用相邻路径点切线差商估算曲率 `κ = wrap_to_pi(Δyaw) / Δs ≤ 1/R_min`；不满足的路径拒绝并对该段重新平滑/重采样路径噪声（不要重跑整张地图）；
- 路径点输出 `(x, y, yaw)`；
