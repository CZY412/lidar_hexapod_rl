#!/usr/bin/env bash
# Run all envelope_cascade_68 tests with the project conda env.
#
# Layout:
...
#
# Closed-loop acceptance (needs the SE2 68-dim policy, currently a reserved
# slot — see envs/el_4090/envelope_cascade_68/checkpoints/README.md):
#   tests/ea2/cascade_68/run_cascade68_closed_loop_test.py
#
...
"${PY}" -m pytest tests/ea2/cascade_68 -q

echo "===== [2/4] env integration: main ====="
"${PY}" tests/ea2/cascade_68/run_cascade68_env_test.py

echo "===== [3/4] env integration: ea2-off fallback ====="
"${PY}" tests/ea2/cascade_68/run_cascade68_env_test.py --ea2-off

echo "===== [4/4] env integration: birth-condition midpoint ====="
"${PY}" tests/ea2/cascade_68/run_cascade68_env_test.py --birth-condition midpoint

echo "===== ALL CASCADE_68 TESTS PASSED ====="
