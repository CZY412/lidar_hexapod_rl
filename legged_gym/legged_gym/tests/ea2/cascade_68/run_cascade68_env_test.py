...
Constructs the registered ``el4090_cascade_68`` env and steps it with small
random locomotion actions (the SE2 checkpoint is NOT required), asserting:

* T0 static contracts: 50 Hz control, 68-dim obs, lidar/heights off,
...
Run:
    python legged_gym/tests/ea2/cascade_68/run_cascade68_env_test.py [flags]
"""
...
if not any(arg.startswith("--task") for arg in sys.argv):
    sys.argv += ["--task", "el4090_cascade_68"]
if "--headless" not in sys.argv:
    sys.argv.append("--headless")

import legged_gym.envs.el_4090.envelope_cascade_68  # noqa: F401,E402  -- registers task
...
def build_env(args, env_cfg):
    ...
    env, _ = task_registry.make_env(name="el4090_cascade_68", args=args, env_cfg=env_cfg)
...
def run_main_variant(env, check):
    ...
    check("SE2 obs dim == 68", int(env.cfg.env.num_observations) == 68)
    ...
    check("initial obs shape", tuple(obs.shape) == (2, 68), str(tuple(obs.shape)))
...
def main() -> int:
    ...
    env_cfg, _ = task_registry.get_cfgs(name="el4090_cascade_68")
